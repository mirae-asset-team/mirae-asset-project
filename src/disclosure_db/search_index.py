    ...
    try:
        ...
        with closing(sqlite3.connect(base_database)) as source_connection:
            ...
            """SELECT fr.evidence_id,fr.filing_id,fr.source_id,
                      f.issuer_name,f.stock_code,f.doc_group,
            ...
            for row in rows:
                ...
                """INSERT INTO search_document(
                    evidence_id,filing_id,source_id,company,stock_code,doc_group,
                    effective_from,effective_to,is_current,lineage_status,fragment_type,locator_json,text_normalized
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    tuple(row),
                )
                indexed_rows += 1
                if str(row[10]) in {"heading", "table_row"}:
                    connection.execute(
                    "INSERT INTO search_trigram(evidence_id,text_normalized) VALUES(?,?)",
                        (str(row[0]), str(row[12])),
                    )
                    trigram_rows += 1
        ...
        connection.execute(
            ...
            (attestation.revision, base_sha256, base_size, datetime.now(UTC).isoformat(), indexed_rows),
        )
...
class SafeSearchIndex:
    def __init__(self, path: Path, *, base_sha256: str):
        self.path = Path(path)
        ...
        try:
            with closing(sqlite3.connect(f"file:{self.path.resolve().as_posix()}?mode=ro", uri=True)) as connection:
                revision = connection.execute(
                    "SELECT base_sha256 FROM index_revision ORDER BY rowid DESC LIMIT 1"
                ).fetchone()
                if revision is None or str(revision[0]).lower() != self.base_sha256:
    ...
    @staticmethod
    def _filters(company: str | None, as_of: str | None) -> tuple[str, list[object]]:
        clauses: list[str] = []
        params: list[object] = []
        if company:
            clauses.append("(d.company=? OR d.stock_code=?)")
    ...
    def _unicode_search(self, token: str, *, company: str | None, as_of: str | None, limit: int) -> list[dict[str, object]]:
        where, params = self._filters(company, as_of)
        with closing(sqlite3.connect(f"file:{self.path.resolve().as_posix()}?mode=ro", uri=True)) as connection:
    ...
    def _trigram_search(self, token: str, *, company: str | None, as_of: str | None, limit: int) -> list[dict[str, object]]:
        where, params = self._filters(company, as_of)
        with closing(sqlite3.connect(f"file:{self.path.resolve().as_posix()}?mode=ro", uri=True)) as connection:
            connection.row_factory = sqlite3.Row
            rows = connection.execute(
    ...
    def search(
        ...
        *,
        ...
        for token in tokens[:32]:
            ranking = self._unicode_search(token, company=company, as_of=as_of, limit=per_token_limit)
        ...
        if len(unicode_ids) < limit:
            for token in tokens[:32]:
                ...
                ranking = self._trigram_search(token, company=company, as_of=as_of, limit=min(remaining, 100))
                rankings.append(ranking)
                unicode_ids.update(str(row["evidence_id"]) for row in ranking)
                if len(unicode_ids) >= limit:
                    break
...
__all__ = ["SEARCH_SCHEMA", "SafeSearchIndex", "SearchIndexBuildResult", "build_search_index", "rrf_fuse"]
