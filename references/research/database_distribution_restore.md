...
포함하지 않습니다. Google Drive 파일을 직접 열거나 여러 명이 동시에 수정하지 않습니다.

## 다운로드 후 검증

Google Drive에서 패키지 전체를 내려받습니다. 압축본은 다음 이름의 62개 조각으로
배포되어 있습니다.

```text
disclosure_corpus_semantic_v1.sqlite.zst.part000
...
disclosure_corpus_semantic_v1.sqlite.zst.part061
```

PowerShell에서 조각을 이름순으로 결합합니다.

```powershell
$output = [System.IO.File]::Create((Join-Path $PWD 'disclosure_corpus_semantic_v1.sqlite.zst'))
try {
  Get-ChildItem -File 'disclosure_corpus_semantic_v1.sqlite.zst.part???' |
    Sort-Object Name |
    ForEach-Object {
...
`zstd`가 설치된 환경에서 Google Drive 동기화 폴더 밖의 작업 폴더로 압축을 풉니다.
...
- Google Drive 공유본에는 팀 DB 담당자만 쓰기 권한을 갖고, 나머지 팀원은 보기 권한만 갖습니다.
