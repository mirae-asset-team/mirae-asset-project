...
## 2026-09-05T02:16:30+09:00 / 2026-09-04T17:16:30Z — 최신 main 재검토와 8000 승격 차단

- 기준점: 원격 `main`과 동일한 `f9b6ad11d644a2c7a40efdb55cf2e8f6aa845182`에서 시작했다. 수정 전 전체 Python 회귀는 `941 passed, 2 skipped, 98 warnings, 260 subtests passed`였고, 공인 `http://101.79.31.221:8000/health`는 HTTP 200, `ready=true`, `company_count=76`이었다. 8001은 15초 안에 응답하지 않았다.
- 운영 재현: 기존 8000은 최신 release identity와 Dense 상태 필드가 없는 이전 이미지였다. 실제 질의에서 자유형·다중근거 경로와 `SM엔터테인먼트와 삼성전자의 영업이익률 비교`가 기대대로 동작하지 않았다. 최신 main을 D 드라이브 base/overlay/search에 read-only로 연결한 로컬 서버에서는 구조화·정책 경로가 개선됐지만 다중 기업 파생비율이 첫 기업을 버리는 오류가 재현됐다.
- Dense 장애 격리 TDD: `compose.release.yaml`의 staging/production 서비스가 `dense-retriever: service_healthy`를 필수 기동 조건으로 둬 Sparse fallback 계약과 충돌했다. release Compose도 Dense health와 무관하게 agent가 시작해야 한다는 테스트를 먼저 추가해 `2 failed`를 확인하고 의존성을 제거했다. focused 결과는 `3 passed, 4 subtests passed`다. 커밋은 `e88854a`다.
- 다중 기업 비율 TDD: alias 정규화된 `에스엠`과 `삼성전자` 각각에 `매출액·영업이익` 두 operand가 필요하다는 router/service 테스트를 먼저 추가해 `2 failed`를 확인했다. 요구조건을 모든 기업×operand로 확장하고, 같은 기업·기간·scope·filing grain 안에서만 Decimal 비율을 계산하며 기업별 결과와 승자를 생성하도록 수정했다. 실제 read-only DB에서 `에스엠 15.58%`, `삼성전자 13.07%`, citation 4건을 확인했다. 커밋은 `0fb3a07`다.
- 검증: 관련 확대 회귀 `90 passed, 60 subtests`, 최종 전체 Python `943 passed, 2 skipped, 98 warnings, 264 subtests passed` (`101.87s`), Web `13/13`, `compileall`, `git diff --check`가 통과했다.
...
## 2026-09-06T17:40:00+09:00 — 제출본 제안서 검수 반영과 답변 문장 결함 수정

- 기준점: 원격 `main` `44c5296`(제출 태그 `submission-2026-09-06` = `0b0d44e` 이후 문서 PR 3건 병합). 수정 전 전체 Python 회귀는 PowerShell에서 `944 passed, 2 skipped, 267 subtests`였다. Git Bash 셸에서 돌리면 `tests/test_release_deployment.py` 94건이 `stdout None` 결합으로 실패하는데, 같은 파일이 PowerShell에서는 `29 passed, 94 subtests`라 셸 환경 차이이지 코드 결함이 아니다.
- 제안서 검수(1차 출처 대조): 대량 QA 12,811건·지연 분포·스테이징 6,091건은 `runs/qa_mass.sqlite`, 300/300·Recall 13/17·856/856·p95 86.94ms는 `contest-release-checklist.md`, 944·9/9는 09-06 핸드오프와 일치했다. 어긋난 서술 4건을 고쳤다. ①§6.3 응답 예시가 재배포 전 결정론 경로 출력이라 09-06 제출 이미지의 외부 실호출 응답으로 교체 ②§8.3 호스트 재부팅 자동 복구 실측은 이전 릴리스(08-20) 것이고 제출 이미지는 컨테이너 재기동만 실측했음을 분리 서술 ③§7.5 Judge Stress V2 `NOT_RUN`을 "제출 빌드 기준 미실행(후보 0f2fb7a 1회 실행은 요청 제한 오류로 판정 불가)"로 정정 ④§7.3 `69개사 × 90여 축`을 원장 실측 `70개사 × 93개 축`으로 정정. API 명세의 `think_trace` 예시와 작성 기준일, README의 팀 저장소 주소(org 이전)도 맞췄다.
- 라이브 `GET /answer` 실호출(09-06)에서 관측한 답변 문장 결함 3종을 TDD로 고쳤다. RED는 `4 failed`(조사 2, 비율 비교 문장 1, 근거 중복 1). ①`부채비율` 같은 2계정 비율은 한 기업·한 기간이라 비교 대상이 없는데 `largest_period`가 채워져 "따라서 2025년 부채비율이 더 큽니다"가 붙었다 — 생산자(`_financial_comparison_response`)에서 파생 연산·단일 기간에는 `largest_period`를 만들지 않고, 렌더러도 기간·기업이 실제로 2개 이상일 때만 비교 문장을 낸다. ②"자본총계은"처럼 고정 조사가 라벨의 받침을 무시했다 — 잎 모듈 `financial_accounts.attach_particle`(은/는·이/가·을/를·과/와·으로/로, 한글 받침·숫자·영문 읽기 기준)을 두고 답변 템플릿 12곳을 전부 바꿨다. 질문 생성기(`financial_release_evaluation`, `judge_stress_v2`)의 문장은 평가 뱅크 identity를 바꾸므로 손대지 않았다. ③같은 공시의 두 근거 셀이 `retrieved_context`에 같은 행으로 두 번 실렸다 — `_official_context`가 공시 단위로 중복을 제거한다.
