...
> 2026-09-05 수익성 분석 보완: 완전한 2개년 수익성 근거를 확보하고도 HCX-005 최종 생성이 20초 timeout이 되는 원인을 재현했다. 검증된 매출액·영업이익·당기순이익으로 최근 두 회계연도의 영업이익률·순이익률 방향을 계산할 수 있는 `profitability` 요청만 `개선/악화/혼재/안정`으로 결정론적으로 판정하고, 모든 admitted citation을 claim verifier로 재검증한 뒤 HCX 없이 반환한다. 중복 fact, 0 매출, 필수 계정·기간 누락은 결론을 만들지 않으며 다른 판단 차원은 기존 HCX/안전 보류 경로를 유지한다. release identity 보완까지 포함한 전체 Python `970 passed, 2 skipped, 266 subtests`, Web `13/13`, release/staging `168 passed, 94 subtests`를 통과했지만 독립 private holdout·hidden 검색 Gold와 provider p95 gate가 남아 8000 승격은 계속 금지다.

> 2026-09-05 release identity 보완: `deploy_staging.ps1`의 exact-image 검사는 `/health.identity`를 요구했지만 runtime에는 commit만 전달되고 API에는 identity 생성 코드가 없어 공식 배포가 구조적으로 실패하는 결함을 RED 2건으로 재현했다. Compose와 staging/release 스크립트가 외부 신뢰 앵커 다섯 값을 전달하고 API는 다섯 값이 모두 유효할 때만 정확한 allowlist를 고정해 노출한다. 누락·형식 오류는 identity 전체를 생략해 gate가 fail-close한다.

> 2026-09-05 최신 staging 인계: commit `7cb6bbf2e805855f4daba0be630e4b87178a1518`, image `sha256:9f0e3ae4fe854ad1a6379e887134040606dd492358668906b824b504b5458e15`가 NCP 8001에서 ready 상태다. provider/function calling, 76개사, 정확한 release identity와 네 read-only data mount를 확인했다. 문제였던 삼성전자 2개년 수익성은 `deterministic_bounded_analysis`, `improved`, citation 6건, final provider 호출 없음, 50.99ms였고 20동시 요청 오류 0·p95 958.35ms였다. 최신 매출은 6.70초, 주입 공격은 provider 호출 전 차단됐다. 외부 8001은 timeout이고 공개 8000은 기존 image로 HTTP 200이다. private holdout·independent hidden Gold 부재 때문에 이 staging smoke만으로 8000 승격하면 안 된다.
