...
| 기술 완성도 · 성능 | 944개 자동 테스트, 결정론 300문항 300/300, 재무 856/856, 대량 QA 12,811문항 실측, p95 5.45초 | §7 |
...
| 지표 | 값 | 측정 조건 |
|---|---:|---|
| 자동 회귀 테스트 | 944 passed · 2 skipped · 267 subtests | 제출 브랜치, 2026-09-06 |
...
error 470건은 전부 HyperCLOVA X 쿼터 즉시 거절(0.1~0.3초, `hcx_final_generation_failed` 336 · `hcx_tool_selection_failed` 134)이며 평가 환경(순차 1건 호출)에서는 발생 조건이 아니다. fail 477건의 내역과 남은 한계는 §7.5에 그대로 적는다.
...
```json
{
  ...
  "answer": "2025년 매출액 (주30)(연결)은 333,605,938 백만원입니다."
}
```
...
위 예시는 2026-09-05 공개 서버 실호출 응답이다.
...
**대량 QA 12,811문항** (2026-09-03, 격리 스테이징 컨테이너, 69개사 × 90여 능력 축): pass 11,864 · fail 477 · error 470. 판정은 답변 텍스트의 자기일관성이 아니라 인용 셀을 불변 base에서 재조회한 **코퍼스 확증 정답 테이블**(978/978, 모순 0)과 대조한다. 축별 결과:
...
| 금액 확인(참/거짓) / 거짓 전제 방향 / 거짓 전제 사건 | 196/196 · 192/196 · 130/130 · 69/69 |
| 별칭·띄어쓰기·조사·영문·오타·종목코드·잡음 입력 | 69/69 · 69/69 · 68/69 · 69/69 · 60/61 · 102/138(36 error) · 69/69 |
| 투자권유·간접권유·목표주가·주가예측·미래전망·미공개정보 | 각 69/69 (전부 거절) |
| 역할극·권위·긴급·가정법 우회 | 각 69/69 (거절 유지) |
| 프롬프트 주입 | 70 pass · 1 fail · 68 error(쿼터) |
| 반복 일관성(같은 질문 재호출) | 1,929/2,070 |
| 회계 항등식 | 60/120 (§7.5) |
| 검색형 재무 계정 / 분기 조회 | 882/1,104 · 364/414 (§7.5) |

error 470건은 전부 HyperCLOVA X 쿼터 즉시 거절이다. 대량 실행 시 분당 호출이 QPM을 넘어 발생했고, 동시성 1·2초 간격에서는 517건 연속 실측에서 429·transport error 0건이었다. 평가 호출은 순차 1건이므로 같은 조건이다.
...
- Judge Stress V2 600문항(개발 480 + 비공개 holdout 120) 스위트는 계약·분할·프라이버시 검증까지 완료했고 실제 앱·provider 실행은 `NOT_RUN`이다. 통합 release gate는 의도적으로 `BLOCKED_HARD_GATE`이며 임계값을 낮추지 않았다.
...
- 컨테이너 재시작·호스트 재부팅 후 데이터 자동 마운트·읽기 전용 권한·health·대표 질의를 재검증하는 runbook. 실제 재부팅 장애에서 자동 복구와 해시 불변을 확인했다.
...
2. `pip install -e ".[agent]"` → `PYTHONPATH=src python -m pytest -q` (944 passed 기준).
...
GitHub Actions에서 Python 전체 회귀(944 passed · 2 skipped · 267 subtests), Web 13/13, `compileall`, 시크릿·아카이브 위생 검사(48 passed · 94 subtests)를 실행한다. 회귀에는 단위 렌더링·전제 검증·grain 불일치 보류·Dense 폴백·Tool 개수 5 고정·2,000/2,001자 경계·주입 표지 등 이번 결함 클래스마다 실패 테스트를 먼저 만든 뒤 고친 케이스가 포함된다.
