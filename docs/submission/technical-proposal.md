...
답변은 `검증된 답변`과 `답변 보류`를 시각적으로 구분한다. 공시 근거는 공시명·공시일·접수번호를 보여주며, 오류는 입력 오류·과다 요청·서버 혼잡·일시 장애로 구분한다. 외부 CDN과 임의 HTML 삽입을 사용하지 않고 CSP와 text-only DOM 렌더링을 적용한다.
...
| 검증 | 결과 |
|---|---:|
| Python 전체 회귀 | 243 passed, 1 skipped, 17 subtests |
| Frontend Node 테스트 | 10/10 passed |
