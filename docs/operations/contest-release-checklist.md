...
| Dense runtime identity contract | PASS_LOCAL_CONTRACT / BLOCKED_ENVIRONMENT | Startup hashes FAISS/metadata against the build manifest, verifies loaded FAISS type/metric and every vector L2 norm, and hashes every mounted model file against staging-generated `/model/model_identity.json`. Manifest and `/health` publish only validated values; existing health fields remain. No new Dense image or deployed health response was validated. |
...
| Full local regression | PASS | Judge Stress V2 Task 1 fix round 2 run `552 passed, 2 skipped, 58 warnings, 74 subtests passed`; warnings are existing FastAPI `on_event` deprecations. |
