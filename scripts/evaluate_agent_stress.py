...
def main() -> None:
    ...
    settings = AgentSettings(
        ...
        attestation_path=args.attestation, use_hcx=False,
    )
    ...
    for case in cases:
        ...
        if checkpoint and should_skip({"question_id": case_id, "input_hash": input_hash}, git_commit, checkpoint):
        ...
        scores.append(score)
        checkpoint = {"case_id": case_id, "input_hash": input_hash, "git_commit": git_commit, "base_sha256": attestation.sha256, "overlay_revision": "semantic-v1-agent-overlay", "search_revision": search_revision, "passed": score.passed, "failures": score.failures, "primary_cause": score.primary_cause, "metrics": score.metrics}
    ...
    summary.update({"run_id": run_id, "case_manifest_sha256": hashlib.sha256(args.cases.read_bytes()).hexdigest(), "provider_configured": agent.provider_configured, "base_sha256": attestation.sha256, "search_revision": search_revision, "completed_count": len(scores)})
